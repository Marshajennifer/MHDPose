...
if args.dataset == 'h36m':
    ...
    parents_np = skel.parents()            # numpy array (N,)
    num_joints = skel.num_joints()         # e.g., 17
...
model_pos_train = MHDPose(args, joints_left, joints_right, is_train=True)
model_pos_test_temp = MHDPose(args,joints_left, joints_right, is_train=False)
model_pos = MHDPose(args,joints_left, joints_right,  is_train=False, num_proposals=args.num_proposals, sampling_timesteps=args.sampling_timesteps)

# attach kinematic-aware RoPE to each model's pose estimator
model_pos_train.pose_estimator.attach_kinrope(parents_np, rope_eps=0.01, joints_left=joints_left, joints_right=joints_right)
model_pos_test_temp.pose_estimator.attach_kinrope(parents_np, rope_eps=0.01, joints_left=joints_left, joints_right=joints_right)
model_pos.pose_estimator.attach_kinrope(parents_np, rope_eps=0.01, joints_left=joints_left, joints_right=joints_right)
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        if newmodel is not None:
            ...
            model_eval = newmodel
            chk_file_path = '/path/to/checkpoint/MHDPose/checkpoint/h36m_best_epoch.bin'
...
if args.render:
    ...
    # draw_3d_image(prediction2_world, ground_truth, dataset.skeleton(), cam['azimuth'], args.viz_subject, args.viz_action, args.viz_camera, frame_to_show=args.viz_frame) # to draw all hypotheses and gt

    draw_3d_image_select(prediction2_world, ground_truth, dataset.skeleton(), cam['azimuth'], args.viz_subject,
                  args.viz_action, args.viz_camera, input_keypoints, poses_2d_reproj, frame_to_show=args.viz_frame) # to draw other combinations
