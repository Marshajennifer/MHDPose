...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., rope_eps=0.05, rope_base=10000.0, comb=False, vis=False):
        super().__init__()
     ...
     # RoPE config + buffers
        self.rope_eps  = float(rope_eps)       # 0 → vanilla
        self.rope_base = float(rope_base)

        self.register_buffer("pos_ids",  None, persistent=True)   # (N,)
        self.register_buffer("phi_bias", None, persistent=True)   # (N,)
    ...
    def set_kinematic_rope(
        ...
        """
        Call once after skeleton is finalized (after remove_joints / re-parenting).
        pos_ids: optional LongTensor (N,) if your token order != joint index.
        pos_ids: token order of your joints
        phi_bias = [0,.1,.2,.3, .1,.2,.3, .1,.2,.3, .4,.3, .4,.5, .3,.4,.5]
        """
        if pos_ids is None:
...
class PMamba(nn.Module):
    ...
    def attach_kinrope(self, parents_np, pos_ids: torch.Tensor | None = None,
        ...
        """
        Call once after the dataset skeleton is finalized.
        parents_np: numpy array of parents (N,)
        pos_ids: optional LongTensor (N,) if your token order != joint index
        """
        ...
        for blk in self.STEblocks:
            ...
            if rope_eps  is not None: blk.attn.rope_eps  = float(rope_eps)
            if rope_base is not None: blk.attn.rope_base = float(rope_base)
    ...
    def forward(self, x_2d, x_3d, t):
        ...
        if self.is_train:
            ...
            x = self.ST_forward(x)                       # (b*h, f, n, cw)
