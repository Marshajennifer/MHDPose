...
from common.pmamba_3dhp import *
...
class MHDPose(nn.Module):
    ...
    def __init__(self, args, joints_left, joints_right, is_train=True, num_proposals=1, sampling_timesteps=1):
        ...
        self.pose_estimator = PMamba(
                    num_frame=self.frames,  # e.g., 27 from command line
                    num_joints=17,                   # Fixed for Human3.6M
                    in_chans=2,                      # 2D joint coordinates
                    embed_dim_ratio=args.cs,    # 512 to 128
                    d_model_temp=args.cs // 2,
                    depth=args.dep,             # Reduce from 8 to 4
                    d_state=32,                       # Mamba state size
                    d_conv=4,                        # Mamba convolution size
                    expand=2,                        # Mamba expansion factor
                    spatial_mlp_ratio=args.spatial_mlp_ratio,
                    temporal_mlp_ratio=args.temporal_mlp_ratio,
                    ste_stride=args.ste_stride,
                    drop_rate=args.dropout,          # e.g., 0.0
                    drop_path_rate=drop_path_rate,   # 0.1 for training, 0 for evaluation
                    is_train=is_train
                )
    ...
    def ddim_sample(self, inputs_2d, inputs_3d, clip_denoised=True, do_postprocess=True):
        ...
        img = torch.randn(shape, device=inputs_2d.device)

        ensemble_score, ensemble_label, ensemble_coord = [], [], []
        x_start = None
        preds_all=[]
        for time, time_next in time_pairs:
            time_cond = torch.full((batch,), time, device=inputs_2d.device, dtype=torch.long)
