...
class MHDPose(nn.Module):
    ...
    def __init__(self, args, joints_left, joints_right, is_train=True, num_proposals=1, sampling_timesteps=1):
        ...
        self.pose_estimator = PMamba(
            num_frame=self.frames,
            num_joints=17,
            in_chans=2,
            embed_dim_ratio=args.cs,       # 512 in example
            d_model_temp=args.cs // 2,     # temporal dim was cs // 4
            depth=args.dep,                # 8 normally, can reduce to 4
            d_state=32,
            d_conv=4,
            expand=2,
    ...
    def ddim_sample(self, inputs_2d, inputs_3d, clip_denoised=True, do_postprocess=True):
        ...
        time_pairs = list(zip(times[:-1], times[1:]))  # [(T-1, T-2), (T-2, T-3), ..., (1, 0), (0, -1)], 0 will be the best
    ...
    def prepare_diffusion_concat(self, pose_3d):
        ...
        x_start = x_start * self.scale # convert GT from mm → scaled-mm

        # noise sample
        x = self.q_sample(x_start=x_start, t=t, noise=noise)

        x = torch.clamp(x, min= -1.1 * self.scale, max= 1.1*self.scale)
        x = x / self.scale # normalised  (≈-1 … 1)      ← sent to MixSTE2
