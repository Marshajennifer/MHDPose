...
def parse_args():
    ...
    parser.add_argument('-awa', '--actionwise-avg', action='store_true', default=False, help='print and log the action-wise averaged MPJPE block')


    # Model arguments
    parser.add_argument('-s', '--stride', default=243, type=int, metavar='N', help='chunk size to use during training')
    parser.add_argument('-e', '--epochs', default=600, type=int, metavar='N', help='number of training epochs')
    ...
    parser.add_argument('-lr', '--learning-rate', default=0.0001, type=float, metavar='LR', help='lr at 140th epoch')
    ...
    parser.add_argument('--spatial-mlp-ratio', default=2.0, type=float, help='MLP expansion ratio for spatial attention blocks')
    parser.add_argument('--temporal-mlp-ratio', default=2.0, type=float, help='MLP expansion ratio for temporal Mamba blocks')
    parser.add_argument('--ste-stride', default=1, type=int, help='apply spatial attention every N ST blocks; temporal blocks still run every layer')
    # parser.add_argument('-dep', default=12, type=int, help='depth of model')
    ...
    parser.add_argument('--distributed', action='store_true',
                        help='use torch.distributed DistributedDataParallel; launch with torchrun')
    ...
    parser.add_argument('--p-agg-mode', default='mean', choices=['mean', 'median', 'trimmed_mean'], help='aggregation rule used for P-Agg evaluation')
    parser.add_argument('--p-agg-trim', type=float, default=0.2, help='trim ratio is used for trimmed_mean in P-Agg aggregation')
    parser.add_argument('--joint-group-eval', action='store_true', help='report Human3.6M torso/intermediate/distal MPJPE and P-MPJPE for P-Agg')
    ...
    parser.add_argument('--viz-frame', type=int, default=None, help='If set, save a single-frame 2D overlay instead of a video.')

# For MPI-INF-3DHP visualization
    parser.add_argument('-sq', '--seq', type=str, default='TS5', help='Sequence name from MPI-INF-3DHP (TS1–TS6)')
    parser.add_argument('-fr', '--frame', type=int, default=0, help='Frame index to visualize')
