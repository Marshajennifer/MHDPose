...
from common.mhdpose_3dhp import *
...
parents_np = np.array(
    [16, 15, 1, 2, 3, 1, 5, 6, 14, 8, 9, 14, 11, 12, -1, 14, 1],
    dtype=np.int64,
)
...
if not args.evaluate:
    ...
    if args.resume:
        ...
        if not args.coverlr:
            lr = checkpoint['lr']
        if args.min_loss == 100000 and 'best_loss' in checkpoint:
            min_loss = checkpoint['best_loss']
            best_epoch = checkpoint.get('best_epoch', best_epoch)
            print('INFO: Resume best validation MPJPE: %.3f mm at epoch %d' % (min_loss, best_epoch))
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            # print("Shape2:", inputs_3d.shape)
            inputs_3d[:, :, 14] = 0
            # print("Shape3:", inputs_3d.shape)
        ...
        if not args.no_eval:
            ...
            valid_mpjpe = losses_3d_valid[-1][0].item()
            if valid_mpjpe < min_loss:
                min_loss = valid_mpjpe
                best_epoch = epoch
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        if newmodel is not None:
            ...
            model_eval = newmodel
            chk_file_path = '/path/to/checkpoint/MHDPose/checkpoint/epoch_60.bin'
        ...
        for _, batch, batch_2d, batch_valid, keys in test_generator.next_epoch():
            ...
            for batch_cnt in range(total_batch):
                ...
                # P-Agg
                mean_pose = aggregate_hypotheses_torch(
                ...
                error = mpjpe_diffusion_3dhp(predicted_3d_pos_single, inputs_3d_single, valid_frame_single.type(torch.bool))
                error_mean = mpjpe_diffusion_3dhp(
                    predicted_3d_pos_single,
                    inputs_3d_single,
                    valid_frame_single.type(torch.bool),
                    mean_pos=True,
        ...
        mat_path_mean = os.path.join(
            args.checkpoint,
            f'inference_data_P_Agg_{args.p_agg_mode}_H{args.num_proposals}_K{args.sampling_timesteps}.mat'
        )
        scio.savemat(mat_path_mean, data_inference_mean)
        mat_path_h_min = os.path.join(
            args.checkpoint,
            f'inference_data_P_Best_H{args.num_proposals}_K{args.sampling_timesteps}.mat'
        )
